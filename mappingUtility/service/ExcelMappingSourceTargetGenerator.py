...
def create_excel_mapping(field_mappings, source_fields, target_fields, source_format, target_format, source_data, target_data):
    """
    ...
    ws_source = wb.create_sheet(title=f"source_{source_format}")
    ws_source.append(["Source Data"])
    ws_source.append([source_data])
    ws_source.sheet_state = "hidden"

    # Add target data sheet
    ws_target = wb.create_sheet(title=f"target_{target_format}")
    ws_target.append(["Target Data"])
    ws_target.append([target_data])
    ws_target.sheet_state = "hidden"
...
def generate_excel_file_with_source_target(source_type, source_data, target_type, target_data):
    ...
    try:
        ...
        output = create_excel_mapping(mappings, source_fields, target_fields, source_type, target_type,source_data, target_data)
