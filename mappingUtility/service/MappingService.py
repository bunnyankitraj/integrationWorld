...
from mappingUtility.strategy.ComponentGeneratorContext import \
...
def process_mapping_files(
    source_data, destination_data, excel_data, source_file_type, destination_file_type
):
    try:
        ...
        destinationTempXml = profile_component_generator(
            destination_data, destination_file_type
        )
        destinationXml = BoomiApiService.upload_xml_component_to_boomi(
            destinationTempXml
        )
        ...
        mapComponentXml = BoomiApiService.upload_xml_component_to_boomi(
            mapTempComponentXml
        )
