...
from mappingUtility.service import (ExcelMappingGenerator, FileTypeChecker,
                                    MappingService)
...
def map_xml_component_generator(request):
    ...
    try:
        ...
        processed_result = MappingService.process_mapping_files(
            source_data,
            destination_data,
            excel_data,
            source_file_type,
        ...
        return JsonResponse(
            {
                ...
                "redirectUrl": "https://platform.boomi.com/AtomSphere.html#build;accountId=dpwsubaccount1-FZOWUA;branchName=main;components="
                + componenetId,
...
def profile_xml_generator(request):
    ...
    try:
        ...
        tb = traceback.format_exc()
        return JsonResponse(
            {"error": str(e), "traceback": tb, "function": "profile_xml_generator"},
            status=500,
        )
...
def mapping_excel_generator(request):
    ...
    try:
        ...
        if not all([source_type, destination_type, source_file, destination_file]):
            return JsonResponse(
                {"error": "Missing one or more required fields or files"}, status=400
            )
        ...
        processed_result = ExcelMappingGenerator.main(
            source_type, source_data, destination_type, target_data
        )
        ...
        tb = traceback.format_exc()
        return JsonResponse(
            {"error": str(e), "traceback": tb, "function": "mapping_excel_generator"},
            status=500,
        )
