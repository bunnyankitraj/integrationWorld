...
def load_segment_structure_from_xml(xml_content):
    ...
    def process_element(elem, path_stack):
        ...
        if tag == "EdiLoop":
            ...
            if not segment_name:
                logger.warning(
                    f"Skipping EdiSegment with no name in XML: {ET.tostring(elem, encoding='unicode')}"
                )
                return
            ...
            segment_structure[segment_name] = {"path": segment_path, "fields": fields}
...
def generate_all_fields_for_used_segments(xml_content, edifact_message):
    ...
    segments_lines = [
        seg.strip() for seg in edifact_message.strip().split("'") if seg.strip()
    ]
    logger.info("Segments found in EDIFACT message:")
    # for s in segments_lines:
    #     logger.info(f"  {s}")

    all_segments_data = defaultdict(
        list
    )  # key=segment_name, value=list of dicts of fields
    ...
    for segment_line in segments_lines:
        ...
        if segment_name not in segment_structure:
            logger.error(
                f"[SKIPPED] Segment '{segment_name}' not found in XML structure"
            )
            continue
...
def get_flat_field_path_list(paths_with_values, segment_structure, edifact_message):
    ...
    segments_in_order = [
        seg.split("+")[0].strip()
        for seg in edifact_message.strip().split("'")
        if seg.strip()
    ]
    seen_paths = set()
    flat_list = []
...
def get_edifact_fields(xml_content, edifact_content):
    ...
    segment_structure = load_segment_structure_from_xml(xml_content)
    paths_with_values = generate_all_fields_for_used_segments(
        xml_content, edifact_content
    )
    ...
    flat_list = get_flat_field_path_list(
        paths_with_values, segment_structure, edifact_content
    )
    return flat_list
