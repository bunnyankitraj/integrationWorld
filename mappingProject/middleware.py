...
class RequestIDMiddleware:
    ...
    def __call__(self, request):
        ...
        try:
            ...
            logger.warning(
                f"[{request.path}] Could not cache request body: {e}",
                extra={"request_id": request_id},
            )
            request._body = b""
        ...
        try:
            ...
            logger.warning(
                f"[{request.path}] Failed to log request data: {e}",
                extra={"request_id": request_id},
            )
        ...
        try:
            ...
            logger.exception(
                f"[{request.path}] Unhandled exception: {e}",
                extra={"request_id": request_id},
            )
        ...
        try:
            ...
            logger.warning(
                f"[{request.path}] Could not log response body: {e}",
                extra={"request_id": request_id},
            )
